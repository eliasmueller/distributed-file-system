...
def basic_multicast(device_info_static: deviceInfo.DeviceInfoStatic, device_info_dynamic: deviceInfo.DeviceInfoDynamic, file_name: str):
    for p in device_info_dynamic.PEERS:
                    if p != device_info_static.PEER_ID:
                        ip = device_info_dynamic.PEER_IP_DICT[p]
                        file_transfer.transfer_file(ip=ip, port=7771, device_info_static=device_info_static, vector_clock=device_info_dynamic.PEER_vector_clock,
                                                    filename=file_name)
